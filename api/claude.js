export default async function handler(req, res){
  ...
  if(!key) return res.status(500).json({error:{message:'Server par ANTHROPIC_API_KEY set nahi hai — Vercel project Settings → Environment Variables mein add karke redeploy karo.'}});
  try{
    ...
    if(!r.ok && payload.model !== 'claude-sonnet-5' && /model/i.test((data.error && data.error.message) || '')){
